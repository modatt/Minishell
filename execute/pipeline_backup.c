...
/*   Updated: 2025/07/23 00:27:03 by modat            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    pid_t *child_pids = malloc(sizeof(pid_t) * cmd_count);
    if (!child_pids) {
...
static void handle_child_process(t_command *current_cmd, t_shell *shell,
                                int prev_pipe_read_fd, int pipe_fds[2])
{
...
static void handle_parent_process(t_command *current_cmd, pid_t pid,
                                  pid_t *child_pids, int *pid_count,
                                  int *prev_pipe_read_fd, int pipe_fds[2])
{
...
void execute_pipeline(t_command *cmd_list, t_shell *shell) 
{
    ...
    while (current_cmd != NULL) {
        ...
        pid = fork();
        if (pid == -1) {
            handle_fork_error(current_cmd, pipe_fds, prev_pipe_read_fd, child_pids);
            return;
        }

        if (pid == 0) {
            handle_child_process(current_cmd, shell, prev_pipe_read_fd, pipe_fds);
        } else {
